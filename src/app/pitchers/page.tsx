...
function PitchersInner() {
  ...
  const [sortBy, setSortBy] = useState<'volume' | 'name'>(
    searchParams.get('sort') === 'name' ? 'name' : 'volume'
  );
        ...
        <div className="border border-border rounded-lg p-4 sm:p-5 bg-card">
          <div className="flex items-start justify-between gap-3 mb-4">
            ...
            <button
              ...
              className="text-muted-foreground hover:text-foreground w-7 h-7 rounded hover:bg-muted flex items-center justify-center text-base leading-none"
            >×</button>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={5} className="px-3 py-6 text-center text-xs text-muted-foreground">No pitchers match the current filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
